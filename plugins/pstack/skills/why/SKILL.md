...
- posture: normal inherited tools with an explicit no-write instruction. Do not use a restrictive sandbox because it may remove connector access, which disables MCP-backed investigators entirely. The source control investigator would be safe in readonly, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
After the Sources Consulted block, if the user's `why` question is a preCodex to actually changing this code, convert the lineage findings into a Preserve / Change / Avoid / Risk constraint set suitable for planning the change.
