...
The parent finds its own transcript file before fanning out. The system prompt names the active workspace's `agent-transcripts/` directory; use that path. Do not glob across `unrelated Codex task-history locations`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
One message, three parallel `spawn_agent` calls, the built-in default subagent, explicit `model:` on each, normal inherited tool access. Reviewers need MCP access for context lookups (tickets, chat threads, observability traces referenced in the transcript); a restrictive sandbox may remove required connector accesss. The prompt forbids file writes; the parent applies edits.
