...
3. Pick the runners. Use `arena runners` from `~/.pstack/codex-models.md` when present. Otherwise default to one each on `gpt-5.6-sol (max)`, `gpt-5.6-sol-max`, `gpt-5.6-terra (medium)`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
