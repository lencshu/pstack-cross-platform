---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via skill-authoring workflow + unslop, optionally pulling fresh evidence from recent transcripts."
...
This skill orchestrates three others: an inline mining pass (see step 1), Claude Code's built-in skill-authoring workflow (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Use Claude Code's skill-authoring workflow to author the skill. Placement:
...
Apply the **unslop** skill and skill-authoring workflow's writing guidelines to every line. Both apply to any agent-read prose, not just skills.
...
A `-mode` skill is subjective output. A skill-authoring workflow-style test/iterate benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill (not working conventions): skill-authoring workflow alone, no mining required.
