...
Before spawning investigators, list the available MCPs from the Claude Code environment. Use the available-tools map when present. Otherwise inspect the `mcps/` directory Claude Code exposes for enabled MCP servers.
...
- posture: normal inherited tools with an explicit no-write instruction. Do not use an overly restrictive tool allowlist because it removes MCP access, which disables MCP-backed investigators entirely. The source control investigator would be safe in readonly, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
- posture: normal inherited tools with an explicit no-write instruction. The synthesizer's quality check spot-verifies citations, which can require MCP access. an overly restrictive tool allowlist removes MCP access and defeats that check.
...
After the Sources Consulted block, if the user's `why` question is a preClaude Code to actually changing this code, convert the lineage findings into a Preserve / Change / Avoid / Risk constraint set suitable for planning the change.
