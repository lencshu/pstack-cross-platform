...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the **skill-authoring workflow** skill (Claude Code's built-in for authoring SKILL.md files).
- Before commit → the bundled `unslop` skill from equivalent Claude Code verification tooling (`/pstack:unslop`).
- Shipping UI / IDE / CLI → use Claude Code's browser/MCP tools for UI surfaces and Bash or PowerShell for CLIs and TUIs. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
- After opening a PR → a bounded PR-check monitoring loop using available GitHub tools.
- Bugbot or the agentic security review commented → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage each finding as fix, dismiss, or ask; then monitor PR checks with available GitHub tools.
- Broken skill mid-task → fix it in its own PR. Don't block. Don't silently work around it.
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "a bounded polling loop until X") → a decision trail via the **show-me-your-work** skill. Commit it when stakes need an auditable record; keep it local otherwise.
...
- **Autonomous run.** A long task to drive to completion without stopping ("run until done", "a bounded polling loop until X"). `playbooks/autonomous-run.md`.
