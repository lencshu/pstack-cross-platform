...
- Prefer `agent type: "poteto-agent"`. `general-purpose` is the fallback. Never use the built-in `plan` agent type; it ignores this skill.
- Pass `model:` explicitly per the configured roles (defaults `sonnet` for code, `opus` for judgment).
...
If a phase creates or edits a skill, the phase instructs the implementer to use the **skill-authoring workflow** skill (Claude Code's built-in for authoring SKILL.md files).
...
- Browser / Electron / Web UIs: the browser or MCP UI-control tools available in Claude Code.
- CLIs and TUIs: Bash or PowerShell control in Claude Code.
...
- a bounded PR-check monitoring loop using available GitHub tools after opening the PR.
