...
3. Pick the runners. Use `arena runners` from `~/.pstack/claude-models.md` when present. Otherwise default to one each on `opus`, `gpt-5.6-sol-max`, `sonnet`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
